...
def update(vehicle_list):
    ...
    update_values = [(vehicle["Lat"], vehicle["Long"],vehicle["status"],vehicle["Odo"],vehicle["EngineStatus"], vehicle["Speed"], vehicle["Plate"]) for vehicle in vehicle_list]
    # Tạo câu SQL UPDATE cho từng bản ghi
    # sql = "UPDATE lf_list_of_truck SET lot_lat = %s, lot_long = %s, lot_tinh_trang_xe = %s, lot_odo = %s, lot_ignition = %s, lot_speed = %s WHERE lot_bien_so_xe = %s"
    sql = "UPDATE lf_list_of_truck SET lot_lat = %s, lot_long = %s, lot_tinh_trang_xe = %s, lot_odo = %s, lot_ignition = %s, lot_speed = %s, lot_status = CASE WHEN lot_status=3 THEN 0 ELSE lot_status END WHERE lot_bien_so_xe = %s"
...
while True:
    ...
    try:
        ...
        for x in data:
            if x["donvi"] == "getThpQuanlyoto" or x["donvi"] == "getTransinQuanlyoto" or x["donvi"] == "getDinhviht":
        ...
        end_time = datetime.datetime.now()  # Ghi lại thời gian kết thúc
        elapsed_time = (end_time - start_time).total_seconds()  # Tính thời gian hoàn thành
        print(f"Thread for completed in {elapsed_time:.2f} seconds at {end_time}")
        #end xử lý chính
        time.sleep(10)
